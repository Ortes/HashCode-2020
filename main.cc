...
int main(int argc, char* argv[]) {
  ...
  std::vector<Library*> filtered_libraries = dataSet.libraries;
  std::copy_if(libraries.begin(), libraries.end(), std::back_inserter(filtered_libraries), [](auto e) -> bool {
    return e->beginSignUpAt != -1;
  });

  for (const auto &library: filtered_libraries) {
